...
public class KeycloakService {
    ...
    public String createUser(String username, String email, String password, String role) {
        ...
        // 3. Assign realm role if provided (e.g., "teacher")
        if (role != null && !role.isBlank()) {
            try {
                assignRealmRoleToUser(userId, role.trim());
            } catch (Exception ex) {
                System.out.println("Warning: failed to assign realm role '" + role + "' to user: " + ex.getMessage());
            }
        }
        return userId;
    }
    ...
    // ===== Realm Role helpers =====
    private Map<String, Object> getRealmRoleByName(String roleName) {
        if (roleName == null || roleName.isBlank()) return null;
        String token = getAdminToken();
        return webClient.get()
                .uri("/admin/realms/" + realm + "/roles/{role}", roleName)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<Map<String, Object>>() {})
                .block();
    ...
    private void assignRealmRoleToUser(String userId, String roleName) {
        Map<String, Object> roleRep = getRealmRoleByName(roleName);
        if (roleRep == null || roleRep.get("id") == null || roleRep.get("name") == null) {
            throw new RuntimeException("Realm role not found: " + roleName);
        }
        String token = getAdminToken();
        java.util.List<Map<String,Object>> roles = java.util.List.of(
                java.util.Map.of(
                        "id", roleRep.get("id"),
